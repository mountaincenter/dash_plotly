...
### 目的
東証大引け後（15:30終了）に、16:45配信向けの市場サマリーレポートを自動生成する。

### ターゲット
一般投資家・トレーダー向けの、4-5分で読み切れる詳細レポート（1,000-2,000文字）
...
**コスト:** ¥3,300/月（Standardプラン固定費に含まれる）
...
|-----|------|----------|-----------|------------------|
| J-Quants Standard | ¥3,300/月 | - | - | ¥3,300 |
...
| 項目 | 月次 | 年間（12ヶ月） |
|-----|------|---------------|
| J-Quants Standard | ¥3,300 | ¥39,600 |
| Grok API | $1.40 | $16.80 |
| **合計** | ¥3,300 + $1.40 | **¥39,600 + $16.80** |
...
**文字数:** 1,000-2,000文字（4-5分読み切り可能）

**セクション構成:**
1. タイトル: `YYYY-MM-DD 国内株式市場サマリー`
2. 主要指数（Markdownテーブル）
3. ボラティリティ（日経VI値 + 水準コメント）
   - 例: `日経VI: 27.70（-3.18%）- 警戒水準`
   - 水準目安: 15-20=平常、20-25=やや警戒、25超=警戒、30超=恐怖
4. セクター動向（箇条書き）
5. 注目ニュース（トップ3、箇条書き）
6. 全体トレンド（箇条書き）
7. 日米主要指標発表予定（箇条書き）
...
- [ ] S3保存機能
- [ ] GitHub Actions統合（毎営業日16:45自動実行）
...
|------|-----------|---------|
| 2025-11-02 | 1.0 | 初版作成 |
| 2025-12-04 | 1.2 | 配信時刻16:00→16:45、文字数上限1,500→2,000(読了4-5分)、J-Quants Light→Standard(¥3,300)、日経VIセクション追加(水準コメント付き) |
