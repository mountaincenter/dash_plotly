...
- プレミアムユーザー言及銘柄が多いほど高品質（加点要素、必須ではない）
"""
...
3. ❓ この出来高・ATR数値はツール結果から取得したか？（推定値ではないか）
4. ❓ この除外確認は正しいか？（Core30リストと照合済みか）
5. ❓ 上場廃止確認をweb_searchで実行したか？（jpx.co.jp、Yahoo!等で確認済みか）

→ **いずれか1つでもNoなら、その銘柄は出力しない**
※ プレミアムユーザー言及は加点要素であり、上記の必須検証項目には含まない
