"""
Grok Prompt v1.3 - Market Summary with J-Quants Data Integration
市場サマリーレポート生成プロンプト（J-Quantsデータ統合版）

Created: 2025-11-02
Updated from v1.2: J-Quants Standard APIのデータを直接組み込み、web_search依存を削減
Description:
    - TOPIX/業種別指数データをJ-Quants APIから取得し、プロンプトに埋め込み
    - web_search()はニュース・トレンド取得のみに限定
    - 数値データの正確性が大幅向上（[確認中]プレースホルダーを削減）
"""
...
def build_market_summary_prompt(context: dict[str, Any]) -> str:
    """
    国内株式市場サマリーレポート生成用のGrokプロンプトを構築（J-Quantsデータ統合版）
    ...
    # プロンプト本体
    prompt = f"""あなたは経験豊富な日本株アナリストです。{latest_trading_day}の東証市場大引け後（15:30終了）時点で、{report_time}配信向けの市場サマリーレポートを作成してください。文字数は1000字以上、1500字程度を目安に（最大2000字以内）。
...
[前場後場の動き・売買代金の段落] 前場は日経平均が[前場引け値]円で引け、[上昇/下落]基調となった。後場は[推移の説明]し、大引けで[終値]円となった。東証プライムの売買代金は前場時点で概算[XX兆XX億円]、大引け時点で[XX兆XX億円]と[活発/低調]な取引となった（出典: [URL1], [URL2]）。

[値上がり下落銘柄数・投資家心理の段落] 東証プライムでは値上がり銘柄が[XX]銘柄、下落が[XX]銘柄、横ばいが[XX]銘柄となり、[全体的な傾向]を示した。投資家心理は[海外勢の動向]、[機関投資家の姿勢]、[個人投資家の動き]といった特徴が見られた（出典: [URL3], [URL4]）。

[為替動向の段落] ドル円レートは午前[時刻]時点で1ドル=[XXX]円、午後[時刻]時点で[XXX]円と推移し、[円安/円高]傾向となった。この為替動向により、輸出セクター（自動車・電機等）は[影響の説明]を受けた（出典: [URL5], [URL6]）。
...
1. **[ニュース見出し1]**: [具体的な内容を2-3文で記述。数値やキーワードを含む。] この動きにより、[市場への影響を具体的に記述]（出典: [URL]）

2. **[ニュース見出し2]**: [具体的な内容を2-3文で記述。数値やキーワードを含む。] この動きにより、[市場への影響を具体的に記述]（出典: [URL]）

3. **[ニュース見出し3]**: [具体的な内容を2-3文で記述。数値やキーワードを含む。] この動きにより、[市場への影響を具体的に記述]（出典: [URL]）
...
3. **Web Search実行**: 上記6カテゴリ（日経平均・全体トレンド・為替・セクター・ニュース・経済指標）のすべてのクエリを実行すること。一部のクエリのみ実行して終了することは厳禁。
...
2. **[確認中]プレースホルダー**: 使用禁止。
3. **データ取得失敗時の省略**: 日経平均・為替動向・経済指標予定（特に本日夜の米国指標）は、データ取得に失敗しても「取得できませんでした」と明記し、セクション自体を削除しないこと。
...
### 品質基準
1. **文字数**: 1000字以上必須、1500字程度を目安に、最大2000字以内厳守。
2. **情報の正確性（最重要）**:
...
4. **網羅性**: 以下のすべてのセクションを含むこと（データ取得失敗時も含む）：
   ...
   - **セクター動向（上昇上位5業種・下落上位5業種の両方とも必須）**: 片方のみの記載は厳禁
   - 注目ニュース（3-4件）
