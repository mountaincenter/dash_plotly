#!/usr/bin/env python3
"""
generate_v2_1_0_2_backtest_report.py

v2.1.0.2 ハイブリッド戦略のバックテストレポート生成
v2.1.0.1 + 買い判定を価格帯別に分析
...
改善点:
- 買い判定を価格帯別（1000円未満、1000-3000円、3000-5000円、5000-10000円）に分けて表示

入力: improvement/data/v2_1_0_comparison_results.parquet
出力: improvement/v2_1_0_2_backtest_report.html
"""
...
OUTPUT_FILE = IMPROVEMENT_DIR / "v2_1_0_2_backtest_report.html"
...
def generate_html_report(df: pd.DataFrame) -> str:
    ...
    # 買いの成績を価格帯別に計算
    def calc_price_range_stats(df_range):
        if len(df_range) == 0:
            return {
                'total': 0, 'wins': 0, 'draws': 0, 'losses': 0,
                'win_rate': 0, 'avg_profit_pct': 0, 'total_profit': 0
            }
        df_no_draw = df_range[df_range['daily_close'] != df_range['buy_price']]
        return {
            'total': len(df_range),
            'wins': df_range['win'].sum(),
            'draws': (df_range['daily_close'] == df_range['buy_price']).sum(),
            'losses': len(df_range) - df_range['win'].sum() - (df_range['daily_close'] == df_range['buy_price']).sum(),
            'win_rate': df_range['win'].sum() / len(df_no_draw) * 100 if len(df_no_draw) > 0 else 0,
            'avg_profit_pct': ((df_range['daily_close'] - df_range['buy_price']) / df_range['buy_price'] * 100).mean(),
            'total_profit': df_range['profit'].sum()
        }

    # 価格帯別に分割
    buy_under_1k = calc_price_range_stats(buy_df[buy_df['buy_price'] < 1000]) if len(buy_df) > 0 else {}
    buy_1k_3k = calc_price_range_stats(buy_df[(buy_df['buy_price'] >= 1000) & (buy_df['buy_price'] < 3000)]) if len(buy_df) > 0 else {}
    buy_3k_5k = calc_price_range_stats(buy_df[(buy_df['buy_price'] >= 3000) & (buy_df['buy_price'] < 5000)]) if len(buy_df) > 0 else {}
    buy_5k_10k = calc_price_range_stats(buy_df[(buy_df['buy_price'] >= 5000) & (buy_df['buy_price'] < 10000)]) if len(buy_df) > 0 else {}
...
<title>V2.1.0.2 バックテストレポート (ハイブリッド戦略 + 価格帯別分析)</title>
<style>
...
<div class="container">
    <div class="header">
        <h1>📊 V2.1.0.2 バックテストレポート</h1>
        <div class="subtitle">ハイブリッド戦略（v2.0.3 ベース + 選択的v2.1.0適用）+ 買い価格帯別分析</div>
    ...
    <div class="summary-section">
        <h2 style="margin-bottom: 20px; color: #667eea; text-align: center;">📈 買い判定 価格帯別成績</h2>
        <div class="summary-grid">
            <div class="summary-card" style="border-left-color: #27ae60;">
                <h3>💎 1,000円未満 ({buy_under_1k.get('total', 0)}銘柄)</h3>
                <div class="stat-row">
                    <span class="stat-label">勝率 (引分除外)</span>
                    <span class="stat-value {'positive' if buy_under_1k.get('win_rate', 0) >= 50 else 'negative'}">{buy_under_1k.get('win_rate', 0):.1f}%</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">平均利益率</span>
                    <span class="stat-value {'positive' if buy_under_1k.get('avg_profit_pct', 0) > 0 else 'negative'}">{buy_under_1k.get('avg_profit_pct', 0):+.2f}%</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">100株利益合計</span>
                    <span class="stat-value {'positive' if buy_under_1k.get('total_profit', 0) > 0 else 'negative'}">{buy_under_1k.get('total_profit', 0):+,.0f}円</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">勝/負/分</span>
                    <span class="stat-value">{buy_under_1k.get('wins', 0)}/{buy_under_1k.get('losses', 0)}/{buy_under_1k.get('draws', 0)}</span>
                </div>
            </div>

            <div class="summary-card" style="border-left-color: #3498db;">
                <h3>💰 1,000-3,000円 ({buy_1k_3k.get('total', 0)}銘柄)</h3>
                <div class="stat-row">
                    <span class="stat-label">勝率 (引分除外)</span>
                    <span class="stat-value {'positive' if buy_1k_3k.get('win_rate', 0) >= 50 else 'negative'}">{buy_1k_3k.get('win_rate', 0):.1f}%</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">平均利益率</span>
                    <span class="stat-value {'positive' if buy_1k_3k.get('avg_profit_pct', 0) > 0 else 'negative'}">{buy_1k_3k.get('avg_profit_pct', 0):+.2f}%</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">100株利益合計</span>
                    <span class="stat-value {'positive' if buy_1k_3k.get('total_profit', 0) > 0 else 'negative'}">{buy_1k_3k.get('total_profit', 0):+,.0f}円</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">勝/負/分</span>
                    <span class="stat-value">{buy_1k_3k.get('wins', 0)}/{buy_1k_3k.get('losses', 0)}/{buy_1k_3k.get('draws', 0)}</span>
                </div>
            </div>

            <div class="summary-card" style="border-left-color: #e67e22;">
                <h3>💵 3,000-5,000円 ({buy_3k_5k.get('total', 0)}銘柄)</h3>
                <div class="stat-row">
                    <span class="stat-label">勝率 (引分除外)</span>
                    <span class="stat-value {'positive' if buy_3k_5k.get('win_rate', 0) >= 50 else 'negative'}">{buy_3k_5k.get('win_rate', 0):.1f}%</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">平均利益率</span>
                    <span class="stat-value {'positive' if buy_3k_5k.get('avg_profit_pct', 0) > 0 else 'negative'}">{buy_3k_5k.get('avg_profit_pct', 0):+.2f}%</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">100株利益合計</span>
                    <span class="stat-value {'positive' if buy_3k_5k.get('total_profit', 0) > 0 else 'negative'}">{buy_3k_5k.get('total_profit', 0):+,.0f}円</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">勝/負/分</span>
                    <span class="stat-value">{buy_3k_5k.get('wins', 0)}/{buy_3k_5k.get('losses', 0)}/{buy_3k_5k.get('draws', 0)}</span>
                </div>
            </div>

            <div class="summary-card" style="border-left-color: #f39c12;">
                <h3>💸 5,000-10,000円 ({buy_5k_10k.get('total', 0)}銘柄)</h3>
                <div class="stat-row">
                    <span class="stat-label">勝率 (引分除外)</span>
                    <span class="stat-value {'positive' if buy_5k_10k.get('win_rate', 0) >= 50 else 'negative'}">{buy_5k_10k.get('win_rate', 0):.1f}%</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">平均利益率</span>
                    <span class="stat-value {'positive' if buy_5k_10k.get('avg_profit_pct', 0) > 0 else 'negative'}">{buy_5k_10k.get('avg_profit_pct', 0):+.2f}%</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">100株利益合計</span>
                    <span class="stat-value {'positive' if buy_5k_10k.get('total_profit', 0) > 0 else 'negative'}">{buy_5k_10k.get('total_profit', 0):+,.0f}円</span>
                </div>
                <div class="stat-row">
                    <span class="stat-label">勝/負/分</span>
                    <span class="stat-value">{buy_5k_10k.get('wins', 0)}/{buy_5k_10k.get('losses', 0)}/{buy_5k_10k.get('draws', 0)}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="summary-section">
        <h2 style="margin-bottom: 20px; color: #667eea; text-align: center;">⏸️ 静観・売り判定</h2>
        <div class="summary-grid">
...
<div style="text-align: center; color: white; padding: 20px; font-size: 0.9em;">
    <p>V2.1.0.2バージョン: v2.0.3ベース + 買い→静観（リスク回避）+ 静観→売り（利益増大）+ 買い価格帯別分析 ｜ 勝敗判定: 買い/静観=利益>0で勝ち、売り=利益<0で勝ち、±0は引分（勝率計算から除外）</p>
</div>
