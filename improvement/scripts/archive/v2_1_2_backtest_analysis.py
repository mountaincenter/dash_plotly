#!/usr/bin/env python3
"""
v2.1.2スコアリングロジックの静的バックテスト分析（精鋭化版）
対象: 政策銘柄 + TOPIX_CORE30（2020-2025）全価格帯
ロジック: strong_buy（25-29点）、strong_sell（-19~-15点）のみ
出力: improvement/v2_1_2_static_backtest_report.html
"""
...
OUTPUT_FILE = BASE_DIR / 'improvement' / 'v2_1_2_backtest_report.html'
...
def calculate_v2_1_2_score(row):
    """
    v2.1.2スコアリングロジック（精鋭化版）
    strong_buy: 25-29点、strong_sell: -19~-15点のみ
...
def backtest_single_ticker(ticker, prices_df):
    ...
    for i in range(len(ticker_df) - 1):  # 翌日が必要なので-1
        ...
        # スコア計算
        score, action, reasons = calculate_v2_1_2_score(row)
...
def main():
    print("=" * 60)
    print("v2.1.2スコアリングロジック 静的バックテスト分析（精鋭化版、2020-2025）")
    print("=" * 60)
    ...
    print(f"  strong_sell: {(results_df['action'] == 'strong_sell').sum():,}件")
    print(f"  hold: {(results_df['action'] == 'hold').sum():,}件")

    # 勝率計算
    buy_df = results_df[results_df['action'] == 'strong_buy']
    sell_df = results_df[results_df['action'] == 'strong_sell']

    if len(buy_df) > 0:
        buy_win_rate = (buy_df['win'] == True).sum() / len(buy_df) * 100
        buy_avg_profit = buy_df['profit_100'].mean()
        print(f"\n  strong_buy勝率: {buy_win_rate:.2f}%")
        print(f"  strong_buy平均利益: {buy_avg_profit:,.0f}円/100株")

    if len(sell_df) > 0:
        sell_win_rate = (sell_df['win'] == True).sum() / len(sell_df) * 100
        sell_avg_profit = sell_df['profit_100'].mean()
        print(f"\n  strong_sell勝率: {sell_win_rate:.2f}%")
        print(f"  strong_sell平均利益: {sell_avg_profit:,.0f}円/100株")

    # [STEP 5] データ保存
    print("\n[STEP 5] データ保存...")
    results_df.to_parquet(BASE_DIR / 'improvement' / 'data' / 'v2_1_2_backtest_results.parquet', index=False)
    print(f"  保存完了: v2_1_2_backtest_results.parquet")
