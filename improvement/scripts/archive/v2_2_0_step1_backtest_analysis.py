#!/usr/bin/env python3
"""
v2.2.0_step1スコアリングロジックの静的バックテスト分析
...
OUTPUT_FILE = OUTPUT_DIR / 'v2_2_0_step1_backtest_results.parquet'
...
def calculate_v2_2_0_step1_score(row):
    """
    v2.2.0_step1スコアリングロジック
    v2.1.1 + MACD
    ...
    macd_histogram_change = row.get('macd_histogram_change')

    if pd.notna(macd_line) and pd.notna(macd_signal):
        # MACDとシグナルの関係
        if macd_line > macd_signal:
            score += 10
            reasons.append(f'MACD ゴールデンクロス')
        elif macd_line < macd_signal:
            score -= 10
            reasons.append(f'MACD デッドクロス')

        # ヒストグラムの変化（モメンタム）
        if pd.notna(macd_histogram_change):
            if macd_histogram_change > 0:
                score += 5
                reasons.append(f'MACD ヒストグラム拡大')
            elif macd_histogram_change < 0:
                score -= 5
                reasons.append(f'MACD ヒストグラム縮小')
...
def backtest_single_ticker(ticker, prices_df):
    ...
    for i in range(len(ticker_df) - 1):  # 翌日が必要なので-1
        ...
        # スコア計算
        score, action, reasons = calculate_v2_2_0_step1_score(row)
...
def main():
    print("=" * 60)
    print("v2.2.0_step1スコアリングロジック 静的バックテスト分析")
    print("v2.1.1 + MACD指標")
    print("=" * 60)
