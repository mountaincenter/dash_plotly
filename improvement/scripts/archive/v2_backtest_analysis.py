...
OUTPUT_FILE = BASE_DIR / 'improvement' / 'v2_1_backtest_report.html'
...
def calculate_v2_1_0_score(row):
    """
    v2.1.0スコアリングロジック（Grokランクなし）
    ...
    if pd.notna(volume_change) and volume_change > 2.0:
        score += 15
        reasons.append(f'出来高{volume_change:.1f}倍（注目急増）')

    # 5日線押し目
    price_vs_sma5 = row.get('price_vs_sma5_pct')
    if pd.notna(price_vs_sma5) and -2.0 < price_vs_sma5 < 0:
        score += 15
        reasons.append(f'5日線押し目{price_vs_sma5:.1f}%（反発期待）')
...
def backtest_single_ticker(ticker, prices_df):
    ...
    for i in range(len(ticker_df) - 1):  # 翌日が必要なので-1
        ...
        # スコア計算
        score, action, reasons = calculate_v2_1_0_score(row)
...
def main():
    print("=" * 60)
    print("v2.1.0スコアリングロジック 静的バックテスト分析（2020-2025）")
    print("=" * 60)
    ...
    # [STEP 5] HTML生成は後で実装
    print("\n[STEP 5] データ保存...")
    results_df.to_parquet(BASE_DIR / 'improvement' / 'data' / 'v2_backtest_results.parquet', index=False)
    print(f"  保存完了: v2_backtest_results.parquet")
