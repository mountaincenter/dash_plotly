#!/usr/bin/env python3
"""
v2.1.4静的バックテスト結果のHTML生成（データドリブン版）
対象: 政策銘柄+CORE30（2020-2025）全価格帯
ロジック: データドリブンスコアリング（実績ベース配点）
出力: improvement/v2_1_4_static_backtest_report.html
"""
...
DATA_FILE = BASE_DIR / 'improvement' / 'data' / 'v2_1_4_backtest_results.parquet'
OUTPUT_FILE = BASE_DIR / 'improvement' / 'v2_1_4_static_backtest_report.html'
...
def main():
    print("=" * 60)
    print("v2.1.4バックテストHTMLレポート生成（データドリブン版）")
    print("=" * 60)
...
<head>
    ...
    <title>v2.1.4 バックテスト分析レポート</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <header>
            <h1>📊 v2.1.4 静的バックテスト分析レポート（データドリブン版）</h1>
            <p>政策銘柄 + TOPIX_CORE30 | 2020-2025 全価格帯 | 実績ベーススコアリング</p>
        </header>
