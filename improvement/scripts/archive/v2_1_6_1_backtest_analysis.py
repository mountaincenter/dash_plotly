#!/usr/bin/env python3
"""
v2.1.6.1スコアリングロジックの静的バックテスト分析
データドリブンスコアリング + 買いシグナルの2段階分類（strong_buy/buy）
対象: 政策銘柄 + TOPIX_CORE30（直近1年: 2024-2025）
"""
...
OUTPUT_FILE = OUTPUT_DIR / 'v2_1_6_1_backtest_results.parquet'

# 設定
MIN_DATA_POINTS = 30
STRONG_BUY_THRESHOLD = 50  # strong_buyの閾値
START_DATE = '2024-01-01'  # 直近1年
...
def main():
    print("=" * 80)
    print("v2.1.6.1スコアリングロジック 静的バックテスト分析")
    print("データドリブンスコアリング + 買いシグナルの2段階分類")
    print(f"分析期間: {START_DATE} 以降（直近1年）")
    ...
    prices_df = prices_df[prices_df['date'] >= START_DATE]  # 直近1年に絞る
