"""
曜日 × RSI/ATR条件 × 信用種別 × 価格帯 の損益分析HTML生成
...
def generate_price_table(df: pd.DataFrame) -> str:
    """価格帯テーブルを生成"""
    html = '''
                                <table class="w-full text-sm">
                                    <thead>
                                        <tr class="text-muted-foreground text-xs border-b border-border/30">
                                            <th class="text-right px-1.5 py-1.5 font-medium whitespace-nowrap">価格帯</th>
                                            <th class="text-right px-1.5 py-1.5 font-medium whitespace-nowrap">件</th>
                                            <th class="text-right px-1.5 py-1.5 font-medium whitespace-nowrap">10:25</th>
                                            <th class="text-right px-1.5 py-1.5 font-medium whitespace-nowrap">%</th>
                                            <th class="text-right px-1.5 py-1.5 font-medium whitespace-nowrap">前場引</th>
                                            <th class="text-right px-1.5 py-1.5 font-medium whitespace-nowrap">%</th>
                                            <th class="text-right px-1.5 py-1.5 font-medium whitespace-nowrap">14:45</th>
                                            <th class="text-right px-1.5 py-1.5 font-medium whitespace-nowrap">%</th>
                                            <th class="text-right px-1.5 py-1.5 font-medium whitespace-nowrap">大引</th>
                                            <th class="text-right px-1.5 py-1.5 font-medium whitespace-nowrap">%</th>
                                        </tr>
                                    </thead>
                                    <tbody>
    ...
    for pr_label, _, _ in PRICE_RANGES:
        ...
        html += f'''
                                        <tr class="border-b border-border/20">
                                            <td class="text-right px-1.5 py-1.5 tabular-nums text-foreground whitespace-nowrap text-xs">{pr_label}</td>
                                            <td class="text-right px-1.5 py-1.5 tabular-nums text-foreground text-xs">{pr['count']}</td>
                                            <td class="text-right px-1.5 py-1.5 tabular-nums whitespace-nowrap text-xs {pr_me_c}">{format_profit(pr['me'])}</td>
                                            <td class="text-right px-1.5 py-1.5 tabular-nums text-xs {winrate_class(pr['win_me'])}">{pr['win_me']}%</td>
                                            <td class="text-right px-1.5 py-1.5 tabular-nums whitespace-nowrap text-xs {pr_p1_c}">{format_profit(pr['p1'])}</td>
                                            <td class="text-right px-1.5 py-1.5 tabular-nums text-xs {winrate_class(pr['win_p1'])}">{pr['win_p1']}%</td>
                                            <td class="text-right px-1.5 py-1.5 tabular-nums whitespace-nowrap text-xs {pr_ae_c}">{format_profit(pr['ae'])}</td>
                                            <td class="text-right px-1.5 py-1.5 tabular-nums text-xs {winrate_class(pr['win_ae'])}">{pr['win_ae']}%</td>
                                            <td class="text-right px-1.5 py-1.5 tabular-nums whitespace-nowrap text-xs {pr_p2_c}">{format_profit(pr['p2'])}</td>
                                            <td class="text-right px-1.5 py-1.5 tabular-nums text-xs {winrate_class(pr['win_p2'])}">{pr['win_p2']}%</td>
                                        </tr>
'''
    html += '''
                                    </tbody>
...
def generate_margin_type_card(df: pd.DataFrame, margin_type: str, margin_label: str, margin_color: str) -> str:
    """信用種別カードを生成"""
    ...
    html = f'''
                            <div class="rounded-lg border border-border/30 bg-card/30 p-3">
                                <div class="flex items-center gap-2 mb-2">
                                    <span class="font-medium text-sm" style="color: {margin_color};">{margin_label}</span>
                                    <span class="text-muted-foreground text-xs">{total['count']}件</span>
                                </div>
                                <div class="flex justify-end gap-3 mb-2 pb-2 border-b border-border/20">
                                    <div class="text-right">
                                        <div class="text-muted-foreground text-xs">10:25</div>
                                        <div class="text-sm font-bold tabular-nums {me_c}">{format_profit(total['me'])}</div>
                                    </div>
                                    <div class="text-right">
                                        <div class="text-muted-foreground text-xs">前引</div>
                                        <div class="text-sm font-bold tabular-nums {p1_c}">{format_profit(total['p1'])}</div>
                                    </div>
                                    <div class="text-right">
                                        <div class="text-muted-foreground text-xs">14:45</div>
                                        <div class="text-sm font-bold tabular-nums {ae_c}">{format_profit(total['ae'])}</div>
                                    </div>
                                    <div class="text-right">
                                        <div class="text-muted-foreground text-xs">大引</div>
                                        <div class="text-sm font-bold tabular-nums {p2_c}">{format_profit(total['p2'])}</div>
                                    </div>
                                </div>
                                {generate_price_table(type_df)}
                            </div>
'''
    return html


def generate_weekday_section(df: pd.DataFrame, weekday: int) -> str:
    """曜日セクションを生成（制度/いちにち/いちにち除0を含む）"""
    ...
    html = f'''
                    <div class="relative overflow-hidden rounded-xl border border-border/40 bg-gradient-to-br from-card/50 via-card/80 to-card/50 p-4 shadow-lg">
                        ...
                        <div class="flex justify-end gap-4 mb-3 pb-3 border-b border-border/30">
                            <div class="text-right">
                            ...
                            <div class="text-right">
                                <div class="text-muted-foreground text-sm">大引け</div>
                                <div class="text-xl font-bold tabular-nums {p2_c}">{format_profit(total['p2'])}</div>
                            </div>
                        </div>
                        <div class="space-y-3">
'''
    for margin_type, margin_label, margin_color in MARGIN_TYPES:
        html += generate_margin_type_card(wd_df, margin_type, margin_label, margin_color)
...
'''
    ...
    for wd in range(5):
        html += generate_weekday_section(seg_df, wd)
...
<head>
<meta charset="UTF-8">
<title>曜日×RSI/ATR×信用種別×価格帯 損益分析</title>
    ...
    <main class="relative min-h-screen">
        ...
        <div class="max-w-[1800px] mx-auto px-4 py-4">
            <header class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-4 pb-3 border-b border-border/30">
                <div>
                    <h1 class="text-xl font-bold text-foreground">曜日×RSI/ATR×信用種別×価格帯 分析</h1>
...
def main():
    ...
    output_path = Path(__file__).parent / 'output' / 'weekday_rsi_atr_price_analysis_v2.html'
