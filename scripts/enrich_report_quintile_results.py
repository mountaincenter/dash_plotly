...
def main() -> None:
    if len(sys.argv) < 2:
        print(f"Usage: python {sys.argv[0]} <YYYYMMDD>", file=sys.stderr)
        sys.exit(1)
    ...
    if idx_start == -1:
        ...
        idx_start = html.find(marker)
    if idx_start == -1:
        print("[ERROR] Q1セクションが見つかりません", file=sys.stderr)
        sys.exit(1)

    section_end = re.search(r"</div>\s*\n\s*<!-- =====", html[idx_start:])
    if not section_end:
        print("[ERROR] セクション終端が見つかりません", file=sys.stderr)
        sys.exit(1)

    idx_end = idx_start + section_end.start()
    new_html = html[:idx_start] + results_html + "\n" + html[idx_end:]
