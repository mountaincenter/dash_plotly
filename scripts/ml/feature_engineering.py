...
=== 目的変数の定義 ===
- phase2_return = (始値 - 終値) / 始値（SHORT基準）
- phase2_win = True: 終値 < 始値（株価下落 = SHORT利益）
- phase2_win = False: 終値 >= 始値（株価上昇または同値 = SHORT非利益）

※ prob_up は 1 - phase2_win を学習するため、値が大きいほど株価上昇確率が高い
"""
...
def calc_price_features(
    ...
    # --- 以下、train_model.py の既存26特徴量に含まれる6特徴量 ---
