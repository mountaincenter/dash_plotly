...
def load_ml_model():
    ...
    if feature_names != FEATURE_COLUMNS or meta.get('n_features') != len(FEATURE_COLUMNS):
        raise ValueError(
            'ML model metadata is not the existing 26-feature schema: '
            f'feature_names={feature_names}, n_features={meta.get("n_features")}'
        )
    model_feature_count = getattr(model, 'n_features_in_', len(FEATURE_COLUMNS))
    if model_feature_count != len(FEATURE_COLUMNS):
        raise ValueError(
            f'ML model expects {model_feature_count} features, expected '
...
def predict_ml_for_stocks(grok_df: pd.DataFrame, model, meta: dict, prices_df: pd.DataFrame, market_data: dict) -> dict:
    """grok_dfの各銘柄に対して既存26特徴量モデルの予測を実行"""
    if model is None:
        return {}
...
def main():
    ...
    if success_count != len(df):
        print("   エラー: 一部銘柄のML予測が欠損したため保存しません")
        return 1
