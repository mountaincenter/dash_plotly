#!/usr/bin/env python3
"""
trading_recommendation.json から
grok_analysis_merged_v2_1.parquet に直接マージ（v2.1ロジック）

Usage:
  python3 scripts/pipeline/merge_json_to_grok_analysis_v2.py
"""
import sys
...
def main():
    print("=" * 60)
    print("Merge JSON files to grok_analysis_merged_v2_1.parquet (v2.1)")
    print("=" * 60)
    ...
    for idx, stock in enumerate(rec_data['stocks'], 1):
        ...
        record = {
            ...
            # 46-49: v2_1 columns (v2.1 logic applied)
            'v2_1_score': v2_1_score,
            'v2_1_action': v2_1_action,
            'v2_1_confidence': v2_1_confidence,
            'v2_1_reasons_json': json.dumps(v2_1_reasons, ensure_ascii=False),
        }
    ...
    print("\n[Step 5] Merging with existing grok_analysis_merged_v2_1.parquet...")
