...
# パス設定
BASE_DIR = Path(__file__).parent.parent.parent  # dash_plotly/
...
def determine_action_comprehensive_v2(row, backtest_stats, fundamentals, price_data, total_stocks):
    ...
    if 5000 <= current_price <= 8000:
        score += 25
        reason_text = f"価格帯5000-8000円（適正範囲）"
        reasons.append(reason_text)
        reasons_structured.append({
            'type': 'price_range',
            'description': reason_text,
    ...
    if 5000 <= current_price < 10000:
        action = '買い'
        confidence = '高'
        reason_text = f"5,000-10,000円範囲（{current_price:,.0f}円）→ロング戦略"
        reasons.append(reason_text)
        reasons_structured.append({
            'type': 'price_5000_10000',
            'description': reason_text,
            'impact': 0  # スコアには影響しないが、判定を上書き
        })
    ...
    # 10,000円超え: 強制ショート
        ...
        action = '売り'
        confidence = '高'
        reason_text = f"10,000円超え（{current_price:,.0f}円）→ショート戦略"
        reasons.append(reason_text)
        reasons_structured.append({
